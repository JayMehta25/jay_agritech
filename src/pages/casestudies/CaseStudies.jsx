...
export default function CaseStudies() {
  ...
  return (
    <GenericPage title={t('pages.case_studies.title')} subtitle={t('pages.case_studies.subtitle')} breadcrumbs={[{ label: t('pages.case_studies.title') }]}>
            ...
            <div className="card" style={{ marginBottom: 'var(--sp-8)', overflow: 'hidden' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }}>
                <div style={{ padding: 'var(--sp-8)' }}>
                  ...
                  <h2 style={{ fontSize: 'var(--fs-h3)', marginBottom: 'var(--sp-4)' }}>{t(`home.case_studies.${study.slug}.title`, study.title)}</h2>
                  <div style={{ marginBottom: 'var(--sp-4)' }}>
                    <h4 style={{ fontSize: 'var(--fs-body-sm)', color: 'var(--clr-text-muted)', marginBottom: 'var(--sp-2)' }}>{t('pages.case_studies.challenge')}</h4>
                    <p>{t(`home.case_studies.${study.slug}.challenge`, study.challenge)}</p>
                  </div>
                  <div style={{ marginBottom: 'var(--sp-4)' }}>
                    <h4 style={{ fontSize: 'var(--fs-body-sm)', color: 'var(--clr-text-muted)', marginBottom: 'var(--sp-2)' }}>{t('pages.case_studies.solution')}</h4>
                    <p>{t(`home.case_studies.${study.slug}.solution`, study.solution)}</p>
                  </div>
                </div>
                <div style={{ background: 'var(--clr-off-white)', padding: 'var(--sp-8)', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <h4 style={{ marginBottom: 'var(--sp-4)' }}>{t('pages.case_studies.results')}</h4>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-3)', marginBottom: 'var(--sp-6)' }}>
                    {t(`home.case_studies.${study.slug}.results`, { returnObjects: true }).map((r, j) => (
                  ...
                  <blockquote style={{ padding: 'var(--sp-4)', background: 'white', borderRadius: 'var(--radius-md)', borderLeft: '3px solid var(--clr-accent-gold)', fontStyle: 'italic', color: 'var(--clr-text-muted)' }}>
                    "{t(`home.case_studies.${study.slug}.testimonial`, study.testimonial)}"
                    <cite style={{ display: 'block', fontStyle: 'normal', fontWeight: 'var(--fw-semibold)', color: 'var(--clr-text-primary)', marginTop: 'var(--sp-2)' }}>— {t(`home.case_studies.${study.slug}.farmer`, study.farmer)}</cite>
                  </blockquote>
                </div>
              </div>
            </div>
